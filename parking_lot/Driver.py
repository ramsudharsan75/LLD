from LLD.parking_lot.dto.CommandDTO import CreateParkingLotCommandDTO, DisplayCommandDTO, ExitCommandDTO, ParkVehicleCommandDTO, UnparkVehicleCommandDTO
        ...
        while True:
            ...
            if parking_lot is None:
                if not isinstance(command_dto, CreateParkingLotCommandDTO):
                    ...
                    parking_lot = ParkingLot(command_dto.parking_lot_name, command_dto.no_of_floors, 
                                 command_dto.no_of_slots_per_floor)
                    print(f"Created parking lot with {command_dto.no_of_floors} floors and {command_dto.no_of_slots_per_floor} slots per floor")
            else:
                if isinstance(command_dto, ParkVehicleCommandDTO):
                    ...
                    if command_dto.display_type == "free_count":
                        parking_lot.display_no_of_free_slots_per_floor_per_vehicle_type(vehicle_type)
                    elif command_dto.display_type == "free_slots":
                        parking_lot.display_all_free_slots_per_floor_per_vehicle_type(vehicle_type)
                    elif command_dto.display_type == "occupied_slots":
                        parking_lot.display_all_booked_slots_per_floor_per_vehicle_type(vehicle_type)
