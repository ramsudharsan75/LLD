...
class ParkingLot:
    def __init__(self, parking_name: str, no_of_floors: int, no_of_slots_per_floor: int) -> None:
    ...
    def park_vehicle(self, vehicle: Vehicle):
        for floor in self.floors:
            for slot_no in range(self.no_of_slots_per_floor):
                ...
                ticket = Ticket(
                    ticket_id=(f"{self.parking_lot_name}_{floor.floor_no}_{slot.slot_no}"),
                    floor=floor,
                    slot=slot,
                    vehicle=vehicle
                )
    ...
    def unpark_vehicle(self, ticked_id: str):
        ...
        print(f"Unparked vehicle with Registration Number: {ticket.vehicle.reg_no} and Color: {ticket.vehicle.color}")

    
    def display_no_of_free_slots_per_floor_per_vehicle_type(self, vehicle_type: VehicleTypeEnum):
        for floor in self.floors:
            print(f"No. of free slots for {vehicle_type.value} on Floor {floor.floor_no}: {len(floor.available_slots[vehicle_type])}")
        
        
    def display_all_free_slots_per_floor_per_vehicle_type(self, vehicle_type: VehicleTypeEnum):
        for floor in self.floors:
            slot_nos = []
            ...
            print(f"Free slots for {vehicle_type.value} on Floor {floor.floor_no}: {",".join(slot_nos)}")
                
    def display_all_booked_slots_per_floor_per_vehicle_type(self, vehicle_type: VehicleTypeEnum):
        for floor in self.floors:
            slot_nos = []
            ...
            print(f"Occupied slots for {vehicle_type.value} on Floor {floor.floor_no}: {",".join(slot_nos)}")
